...
Любое изменение, влияющее на продовое поведение AI-ассистента, проверяется локально:

1. запустить проект на `localhost`;
2. открыть виджет в браузере;
3. провести живой диалог через интерфейс;
...
8. сохранить протокол в `local-live-tests/*.local.md`.

Без успешной локальной живой проверки изменение поведения не считается готовым.
