...
Follow-up implemented: the code default for `OPENAI_HEADLESS_DAILY_TOKEN_BUDGET` was raised from `160000` to `1600000`. This budget applies only to headless production live tests detected as `production_live_test`; real buyers on the production widget are governed by the separate `OPENAI_DAILY_TOKEN_BUDGET`.
