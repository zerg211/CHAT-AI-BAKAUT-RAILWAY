...
## Дополнение 2026-05-13: свежий диапазон не должен запираться старой подборкой

Повторный production-прогон через `bakautprof.ru` после phase/lead фикса показал, что Turn 5 стал лучше: ложное `трехфазный 230/400 В` исчезло, контакт не навязывался, ответ был от первого лица. Но buyer-view аудит выявил оставшуюся проблему:

- на Turn 4 `А что есть в наличии от 8 до 10 кВт?` бот показал `ТСС SGG 10000EHA` и `ТСС SGG 12000EHLA`;
- `12 кВт` не должно было быть карточкой под диапазон `8–10 кВт`;
- `ТСС SGG 9000ELA (8 кВт)` и `ТСС SGG 10000EI (9 кВт)` были в каталоге и появились только после возражения `Нет, просто покажите варианты`;
- технически metadata показала, что старое требование `10 кВт` осталось активным, а `searchScope=previousSelectionOnly` запер executor внутри прежних карточек `10/12 кВт`.

Исправление:
- если покупатель в текущем ходу дает явный числовой диапазон мощности, этот диапазон перебивает старое одиночное значение мощности даже при semantic selection state;
- если LLM просит `find_matching_products`, но одновременно ставит `previousSelectionOnly` без конкретных `selectedProductIds`, а текущий ход содержит новый числовой диапазон/лимит, executor не запирается в прежних карточках и снова смотрит каталог;
- в trace добавлен `stalePreviousSelectionCageRepaired`, чтобы metadata-аудит видел такой repair;
- live-тест #876 теперь считает только новые карточки текущего хода, а не все карточки страницы, и падает, если в подборе `8–10 кВт` показан `12 кВт` или пропущены ожидаемые `SGG 9000ELA / SGG 10000EI / SGG 10000EHA`.

Ожидаемый результат:
- на запрос `ТСС 8–10 кВт 220` бот должен сразу показать все подходящие карточки из каталога: 8, 9 и 10 кВт;
- старое состояние `10 кВт` может быть контекстом, но не должно сужать новый диапазон;
- `previousSelectionOnly` остается допустимым только когда покупатель реально выбирает из уже показанных карточек, а не задает новый/расширенный каталоговый подбор.
