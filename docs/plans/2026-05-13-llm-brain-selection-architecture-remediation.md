...
## Дополнение 2026-05-13: alternatives не могут нарушать hard constraints

Следующий production-прогон упал уже раньше:

- на `Есть в наличии ТСС 10 кВт бензин?` видимые карточки оказались `SGG 8000EH3NUA` и `SGG 17000EH3U`, хотя текст сам писал, что 10 кВт есть под `Показать еще`;
- после уточнения `Нужен 220 В, однофазный` видимые карточки снова включали трехфазные/неподходящие товары;
- на `8–10 кВт` бот сказал: `первый видимый вариант SGG 8000EH3NUA, но он трехфазный, не 220 В`.

Причина:
- `nearestCatalogShortlistAlternatives` мог добавлять ближайшие альтернативы по мощности/цене, но не блокировал нарушение жесткого `singlePhase220=true`;
- потом card selection считал эти alternatives частью `structured_selection`, и текст пытался объяснить неподходящую первую карточку вместо того, чтобы показать подходящие 220 В карточки.

Исправление:
- alternatives для генераторов теперь не проходят, если покупатель явно задал `singlePhase220=true`, а товар определяется как `mixed_220_380` или `three_phase_380`;
- live-тест дополнительно падает уже на Turn 2/3, если visible cards под `ТСС 10 кВт` или `220 В однофазный` показывают 8 кВт/15,5 кВт/трехфазные товары вместо 10 кВт 220 В.

## Дополнение 2026-05-13: старый бренд из сравнения не должен перебивать текущий бренд подбора

Следующий production-прогон показал еще одну причину фразы `точного ТСС 10 кВт 220 не вижу`:

- в текущем LLM-плане бренд был `ТСС`;
- но в `hardConstraints.brandConstraint` попал `Doosan` из старой темы сравнения двигателей;
- из-за этого executor отбрасывал `ТСС SGG 10000EHA`, хотя карточка была в каталоге.

Исправление:
- `applySemanticMemoryToSelectionState` больше не перезаписывает уже выставленный current-turn `brandConstraint` брендом из semantic memory;
- текущий LLM-план/текущая реплика покупателя сильнее старой памяти, особенно когда старый бренд был частью сравнения, а не целевого подбора;
- добавлен unit-тест: stale `Doosan` в semantic memory не мешает подобрать `ТСС SGG 10000EHA` по текущему запросу.

## Дополнение 2026-05-13: hidden карточки нельзя называть без модели

Следующий production-прогон дошел до корректных 8/9 кВт карточек, но на Turn 4 текст сказал только `10 кВт тоже есть под Показать еще`, не назвав `SGG 10000EHA`.

Исправление:
- если ответ уже упоминает `Показать еще`, но не называет скрытые карточки, guard добавляет конкретные названия первых hidden cards;
- покупатель должен видеть в тексте, какая именно модель скрыта за кнопкой, иначе он снова получает неполный ответ.
