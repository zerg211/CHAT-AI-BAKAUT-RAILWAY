...
function buyerAudit(step) {
  ...
  if (step.phase === 'vibroplate_1000kg') {
    ...
    if (cards.some((card) => /(?:\b[1-7]\d\s*кг\b|\b8\d\s*кг\b|\b9\d\s*кг\b|1[01]\d\s*кг|120\s*кг)/iu.test(card))) {
      issues.push('показал легкие виброплиты как подходящие на запрос около 1000 кг');
    }
