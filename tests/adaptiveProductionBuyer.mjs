...
function fallbackDecision({ goal, steps, turnIndex }) {
  ...
  if (assistantAsksPumpClarification(lastAssistant) && !coverage.answeredPumpDetails) {
    return {
  ...
  if (!coverage.askedPlateNeed) {
    return {
      ...
      user: 'Еще нужна виброплита для въезда под плитку. Там песок и щебень, площадь небольшая, грузить буду сам. Какой вес смотреть?',
  ...
  if (!coverage.askedDelivery) {
    return {
      ...
      user: 'Если брать генератор и виброплиту, доставка до Азова у вас бывает? И наличие по этим позициям можно уточнить?',
