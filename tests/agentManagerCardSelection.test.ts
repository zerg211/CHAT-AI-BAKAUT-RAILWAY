...
function structuredSelectionIntentForClass(
  productClass: NonNullable<AgentSelectionPolicy['canonicalProductClass']>,
  policy: Partial<AgentSelectionPolicy> = {}
): AgentIntentContract {
  const intent = structuredSelectionIntent({
    targetProductClass: productClass,
    canonicalProductClass: productClass,
    ...policy
  });
  intent.toolRequests = intent.toolRequests.map((request) => ({
    ...request,
    args: {
      ...request.args,
      productIntent: productClass,
      canonicalProductIntent: productClass,
      query: productClass
    }
  }));
  return intent;
...
describe('AgentManager visible card readiness', () => {
  ...
  it('filters visible generator cards to battery stations when the structured need requires battery power', () => {
    ...
    const selection = selectProductsForVisibleCards({
      ...
      history: [],
      intent: structuredSelectionIntentForClass('generator', { powerSource: 'battery' }),
      answerText: 'Here are the relevant generator options.',
      selectedProductIds: [gasoline.id, diesel.id, battery.id],
      needState
    });
    ...
    const selection = selectProductsForVisibleCards({
      ...
      intent: structuredSelectionIntentForClass('generator', {
        ...
        requirements: [{
          ...
          role: 'hard_constraint',
          strictness: 'strict',
          evidence: 'buyer requires at least 800 W',
          verification: { mode: 'product_attribute' }
        }]
      }),
      ...
      selectedProductIds: [aps600.id, aps800.id, aps1800.id],
      needState
    });
    ...
    const selection = selectProductsForVisibleCards({
      ...
      intent: structuredSelectionIntentForClass('generator', {
        ...
        requirements: [{
          ...
          role: 'hard_constraint',
          strictness: 'strict',
          evidence: 'buyer requires 800 W',
          verification: { mode: 'product_attribute' }
        }]
      }),
      answerText: 'APS800 fits. APS600 is weaker than the request.',
      selectedProductIds: [aps800.id, aps600.id],
      needState
    });
  ...
  it('does not override writer-selected material candidates with keyword matching', () => {
    ...
    const selection = selectProductsForVisibleCards({
      ...
      history: [],
      intent: structuredSelectionIntentForClass('diamondBlade', {
        requirements: [{
          id: 'porcelain-material',
          kind: 'material',
          value: 'porcelain_tile',
          unit: null,
          relation: 'must_have',
          role: 'hard_constraint',
          strictness: 'strict',
          evidence: 'buyer needs a blade for porcelain tile',
          verification: { mode: 'product_attribute' }
        }]
      }),
      answerText: 'The porcelain blade fits. The concrete blade is for concrete, not porcelain tile.',
      selectedProductIds: [porcelain.id, concrete.id],
      needState: needStateWithBudget()
    });
  ...
  it('suppresses previous 400 kg plate cards when the current task is home paving tile', () => {
    ...
    const selection = selectProductsForVisibleCards({
      ...
      history: [],
      intent: structuredSelectionIntentForClass('plate', {
        reusePreviousCards: true,
        requirements: [{
          id: 'home-paving-weight',
          kind: 'weight_max_kg',
          value: 100,
          unit: 'kg',
          relation: 'must_have',
          role: 'hard_constraint',
          strictness: 'strict',
          evidence: 'semantic decision bounds the home paving class to at most 100 kg',
          verification: { mode: 'product_attribute' }
        }]
      }),
      answerText: 'Husqvarna LG 400 is lighter than the rest.',
      selectedProductIds: [grost.id, masterpac.id, husqvarna.id],
      needState: needStateWithBudget(),
    ...
    expect(selection.warnings).toContain('product_cards_suppressed:structured_weight_no_fit');
  });
    ...
    const selection = selectProductsForVisibleCards({
      ...
      intent: structuredSelectionIntentForClass('plate', {
        ...
        requirements: [{
          ...
          role: 'hard_constraint',
          strictness: 'strict',
          evidence: 'buyer explicitly needs about 400 kg',
          verification: { mode: 'product_attribute' }
        }]
      }),
      answerText: 'GROST VH 400D and Husqvarna LG 400 match.',
      selectedProductIds: [grost.id, husqvarna.id],
      needState: needStateWithBudget(),
      allowHistoricalProducts: true
    });
  ...
  it('keeps only products with explicit no-autostart facts for an explicit must-not-have requirement', () => {
    ...
    expect(selectionWithoutExplicitIds.selectedProductIds).toEqual([]);
    expect(selectionWithoutExplicitIds.warnings).toContain('product_cards_blocked:missing_structured_llm_selection');
  });
  ...
  it('accepts a typed strict gasoline requirement and removes diesel generator cards', () => {
    ...
    expect(selection.warnings).toContain('product_cards_filtered_by_fuel:gasoline:1');
  });
