...
function execute(){const temp=fs.mkdtempSync(path.join(os.tmpdir(),'bakaut-oracle-mutants-'));const results=[];
 try{for(const [id,from,to]of variants){const target=path.join(temp,id);fs.mkdirSync(path.join(target,'evals'),{recursive:true});fs.mkdirSync(path.join(target,'tests'),{recursive:true});fs.cpSync(path.join(root,'evals/acceptance'),path.join(target,'evals/acceptance'),{recursive:true});fs.cpSync(path.join(root,'tests/acceptance'),path.join(target,'tests/acceptance'),{recursive:true});
