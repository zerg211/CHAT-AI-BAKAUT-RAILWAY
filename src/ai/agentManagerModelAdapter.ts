...
export function plannerSystemPromptBlock(
  ...
  return [
    ...
    'Для сравнения точных моделей, где вывод зависит от свежести источника или условий измерения, ставь sourcePolicy="web_required" и webRequirement="independent_required". В web.researchProductFacts включай все grounding.technicalAttributes, численные значения которых собираешься сопоставлять, а не только первый пробел каталога. Для показателей, зависящих от методики (уровень шума, вибрация, расход, производительность под нагрузкой и подобные измерения), запроси также тип показателя, стандарт, расстояние, режим нагрузки и иные условия, нужные для сопоставимости. Если сопоставимость не подтверждена, планируй ответ с раздельными заявленными значениями без вывода о практическом превосходстве.',
  ...
  async decideTurn(input: AgentManagerModelInput): Promise<AgentSemanticDecision> {
    ...
    const issueGuidance = [
      ...
      hasIssue('required_web_tool_missing') || hasIssue('required_tool_request_missing:web.researchProductFacts') || hasIssue('conditional_research_plan_missing')
        ...
        : '',
      hasIssue('external_url_misrouted_to_first_party_reader') || hasIssue('external_evidence_url_requires_web_research')
        ? 'Внешний URL нельзя читать через site.readFirstPartyPage: этот инструмент принимает только bakautprof.ru. Сохрани запрос покупателя на проверку внешнего источника, поставь buyerRequestedWeb=true, sourcePolicy="web_required", webRequirement="buyer_requested", requiredToolKinds с web.researchProductFacts и добавь required web.researchProductFacts. Включи внешний URL в query, точные модели — в productNames, проверяемые характеристики — в technicalAttributes/comparisonAttributes; отдельную ссылку bakautprof.ru оставь для site.readFirstPartyPage.'
        : '',
    ...
    const { parsed } = await createStructuredJsonResponse({
      request: {
        ...
        input: [{ role: 'system', content: [
          ...
          'Для численного сравнения точных моделей проверь, что все сравниваемые grounding.technicalAttributes покрыты требуемым свежим источником. Разные измерительные источники могут подтверждать сами числа, но не их сопоставимость: без одинакового типа показателя, стандарта, расстояния, нагрузки и существенных условий продолжи исследование либо ответь отдельными заявленными значениями без вывода о практическом превосходстве.',
      ...
      input: [
        {
          ...
          content: [
            ...
            'В сравнениях и превосходной степени называй точный измеряемый показатель и доказанную область найденных результатов: например, «наибольшая найденная номинальная мощность». Для catalog generatorLoadFit.ranking используй готовые scope, metric, referenceKw, orderedProductIds и deltaAboveMinimumKw как авторитетный числовой порядок; не выводи другую точку сравнения из query/semanticQuery. В customer text и selectionRationale лучше назвать номинал и готовую дельту. Не превращай внутренний порядок в глобальное «ближайший/лучший в каталоге», если scope ограничен returned_eligible_candidates. Не объединяй номинальную, максимальную, пусковую, активную и полную мощность в общее «самый мощный», если evidence не доказывает один и тот же показатель.',
            ...
            'calculator.generatorLoad ok: payload.profile.requiredNominalKw/requiredStartingKw — расчётный минимум только когда эти поля присутствуют и missingStartingLoads пуст. При неизвестном пуске totalRunningKw/runningOnlyNominalFloorKw описывают лишь работу без учёта пуска, а не достаточный минимум генератора. Каждый элемент missingStartingLoads остаётся отдельным блокирующим фактом: один заданный покупателю вопрос — только следующий шаг, если элементов несколько; он не делает подбор точным сам по себе. Среди достаточных по мощности вариантов соблюдай порядок rankingObjectives покупателя. Без явного rankingObjective/remote-start catalog generatorLoadFit.ranking уже задаёт порядок по минимальному подтверждённому номиналу не ниже referenceKw; не пересчитывай его из query и не называй этот ограниченный порядок глобальным превосходством. В тексте называй абсолютные номиналы и готовые deltaAboveMinimumKw. Превышение >1.5× — на позиции 2+ с числами в тексте (+X кВт к расчёту, +Y руб, зачем); слова запас/комфорт/надёжность/ресурс/бренд/дизель без этих чисел — не обоснование. Тип топлива, бренд и ресурс requiredNominalKw не меняют. Топливо покупателем не заявлено — смешанный показ топлив либо явная оговорка «показываю только [топливо], потому что [причина]; нужно другое — скажите». Оценки — «по расчету/допущениям», отдельно назови какой факт (шильдик насоса/инструмента) нужен следующим, не называя его единственным, если остаются другие пробелы. not_found — не выдумывай кВт. Warnings estimate_only/unbounded_guess/invalid_load_kind/bounded_basis_incomplete/bounded_assumption: без final fit и без утверждения совместимости; browse_catalog может показывать ассортимент без обещания совместимости. preliminary_fit может показывать предварительные варианты с canShowProductCards=true только при минимум одном заявленном требовании покупателя и без доказанного конфликта, а missingFacts и answerText точно называют непроверенную нагрузку. Estimate-only с нулем заявленных требований — это needs_more_info: canShowProductCards=false, selectedProductIds=[], короткая ориентация по классу как явно грубая (не факт о товаре) и ровно один главный вопрос, без карточек. final_fit — canShowProductCards=false и минимальный вопрос.',
