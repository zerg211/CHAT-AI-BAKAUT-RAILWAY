...
Верни только валидный JSON по схеме:
{
  ...
  "semanticMemory": {
    ...
    "requirements": [{
      "id": string,
      "kind": "productClass" | "task" | "weightKg" | "budgetRub" | "powerKw" | "diameterMm" | "brand" | "fuel" | "startType" | "phase",
...
- If the buyer explicitly needs easy generator start, electric start, key start, button start, or no cord pulling, store it as semanticMemory.requirements kind="startType", value.text="electric", source="explicit_user", strictness="strictOnly"; also put startType="electric" in selectionState.hardConstraints. Do not store this requirement when it is only the assistant's inferred preference.
...
When the buyer explicitly names brand, fuel type, start type, phase/voltage, product class, or a numeric range, preserve it as a hard constraint in both selectionState.hardConstraints and semanticMemory.requirements. Use semanticMemory kind "brand", "fuel", "startType", "phase", "productClass", or the relevant numeric kind instead of leaving the fact only in free text. Do not broaden away an explicit brand/fuel/start/phase/range unless the buyer changes it.
