...
When the buyer asks what catalog options exist/are available for the current product need, treat it as catalog selection with commercial availability verification: answer from catalog matches, show matching product cards when the catalog has them, and separately say live warehouse availability is manager-verified. Use pure_availability only for a warehouse/stock fact or exact product presence check, not to suppress cards for a range/list request.
Decision boundary for availability vs selection:
- If the buyer asks whether one exact named model/article/product exists or is in stock, use taskType="pure_availability"; use catalogAction="exact_model_lookup" when the model should be checked, or "verify_catalog_absence" only after catalog evidence is insufficient; productCardsPolicy may be "show_exact_matches" when exact catalog cards are useful.
...
- If the buyer adds delivery to a product selection request, use taskType="product_selection_with_delivery", keep catalogAction="find_matching_products" and productCardsPolicy="show_matching_products"; answer delivery as manager/logistics verification without contact pressure unless the buyer is ready to leave a request.
