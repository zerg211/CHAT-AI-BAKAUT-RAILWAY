...
- Final repair removed the temporary start-family matcher and restored full coverage: cached facts skip web verification only when stored attributes cover every requested structured attribute. Broader attribute paraphrases need a future LLM normalization pass, not a code dictionary or partial-coverage shortcut.
...
- Final `npx vitest run tests/agentManagerComparisonResearch.test.ts` after restoring full verified fact coverage - PASS, 13 tests.
...
- Final live widget full-coverage check: PASS, protocol `local-live-tests/2026-05-22-verified-fact-full-coverage-db81c9c.production.md`.
...
- AC3: PASS. Agent Manager checks local verified facts first and skips web only when stored exact-model facts cover every requested structured attribute.
