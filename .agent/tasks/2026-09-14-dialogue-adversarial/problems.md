...
## P12 — reviewer invented a comparison reference and stale card order controlled web targets

PR #33 merged as `c785cb5fb6910d5c34813c7cc97284aa11cb1deb`; Railway deployment `d7a30d48-87c6-4ab9-be27-c574745f7923` reached SUCCESS at that exact SHA and the public health marker matched. Clean production conversation #2193 (session `f663e763-6ace-4358-8944-e00d1866b138`, turn `76ab31ce-1ecc-43ab-873e-eb4017f05211`) again committed no assistant message. Captures are `raw/production-dialogue-2026-09-15-failed8-admin.json`, `raw/production-dialogue-2026-09-15-failed8-turn-artifacts.json` and `raw/production-dialogue-2026-09-15-failed8-turn1.txt`.

The catalog and calculation were now timely and correct: the 3 kW running-only floor removed underpowered products and catalog search completed in 8.571 seconds. The writer produced a useful preliminary answer, but called DAEWOO 3.4 kW the nearest candidate. The reviewer twice rejected it by claiming that ZONGSHEN 4.0 kW was closer to the 2.9 kW running load. That arithmetic is false: the absolute deltas are 0.5 and 1.1 kW. The reviewer had also inferred an unsupported 4–6 kW reference from the search query instead of using the calculator's proven floor.

The same trace exposed an ordering gap. The broad catalog result arrived as 3.4, 8, 7, 17, 4, 7, 10 and 6.6 kW. Automatic web targets used the first four map entries, so the limited web budget checked oversized candidates before 4 and 6.6 kW. In later turns, `priorProducts` could keep an even older insertion order because `Map.set` does not move an existing key.

Final root/critic resolution:

- deterministic default ranking uses only `requiredNominalKw`, otherwise the validated `runningOnlyNominalFloorKw`; among eligible candidates it orders known nominal power ascending and keeps unknown nominal power last;
- `filterProductsByStructuredSelectionPolicy` runs after the default load order, preserving its confirmed-before-unconfirmed evidence partition while retaining ascending power inside each tier;
- an explicit structured price or nominal ranking objective and a remote-start preference retain ownership of their own order;
- `generatorLoadFit.ranking` publishes the exact `returned_eligible_candidates` scope, metric, reference, basis, final product IDs, nominal values and precomputed deltas;
- writer and reviewer accept that metadata and cannot infer another comparison point from `query` or `semanticQuery`; global closest/best language remains unsupported for a limited result scope;
- automatic web targets use the latest successful matching catalog result order, restricted to current eligible IDs, rather than stale `productsById` insertion order.

The critic first blocked the implementation because default load sorting after structured filtering could break confirmed-before-unconfirmed order. After that was corrected, the critic found the stale-map web target counterexample and required an executor regression with prior products in a conflicting order. Both corrections were accepted. Current focused verification passes 226/226 tests; TypeScript, production build, no-new-regex and diff checks pass. The contention failures from the parallel whole-suite run all pass in an isolated one-worker rerun (118/118). Publication and a new adaptive production dialogue remain pending.

## P13 — catalog latency exposed incomplete recovery and reviewer scope

PR #34 merged as `5c616d44172e4a5472fdc93b74dcc4d7fed00a3c`; Railway deployment `738da570-1297-416a-a8e3-595ba5f439a8` reached SUCCESS and the public marker matched. Production conversation #2194 preserved the correct 2.9 kW calculation but failed to show preliminary products. The main catalog call timed out on all three buyer attempts. The second and third turns then failed answer review, so the buyer saw the generic completion error twice.

Production timing isolated the deterministic bottleneck. Structured selection reads 200 initial and up to 1,000 expansion candidates. `searchProducts` performed substring scans of full `description` and `specs` twice even though both fields are already covered by the GIN-backed `search_tsv`. Direct production probes measured the former reads at 3.3–14.5 seconds and about 6.5 MB combined; retaining full-text coverage while limiting substring fallback to name, category and source URL reduced the reads to about 0.7–0.9 seconds each. Root proposed a 20-second timeout; the critic rejected it because a late catalog start would leave web near its 6-second execution floor. The critic's argued 15-second ceiling is the final shared decision.

The failed fallback exposed two independent scope defects. Web recovery called `searchCatalogProducts` without the primary selection intent, calculator results or absolute deadline, so it bypassed the running floor and phase/fuel constraints. Reviewer input also omitted prior ledger facts and product roles while preserving nested rejected web products. It therefore called valid prior requirements unsupported and promoted a 2.8 kW and a three-phase reference to available recommendations. The final correction passes the selection contract into recovery, derives automatic targets only from the filtered ranked result, removes all nested web products from model-facing tool results, and supplies ledger plus exhaustive product roles to reviewer.

Six deterministic `fact_evidence_product_mismatch` issues had another cause: calculator evidence items are scalar and productless, while the correct writer named the refrigerator, pump and grinder. The resolver now infers a name only from the canonical calculator load item or `missingStartingLoads` subject when a fact is named. Wrong consumer, equal-number cross-consumer, named aggregate and wrong missing-subject cases remain blocked.

The critic initially returned FAIL, required these three corrections and four negative binding cases, then independently reviewed the finished diff. Its fresh verdict is PASS: 134/134 tests, TypeScript, no-new-regex and diff checks all passed. Root broadened the relevant suite to 354/354 and passed typecheck, build and the regex guard. Publication and a new production dialogue remain pending.

## P14 — preliminary fit could still rank a complete oversized card above a closer incomplete card

The user reopened production conversation #2188 and correctly challenged the visible A-iPower 6 kW plus FUBAG 12 kW shortlist. A fresh read of turn `45a3aa5d-a1f7-4863-887e-c555ae2ef25e` confirmed that the old catalog result already contained Zongshen/EVOline 4 kW and EVOline 5 kW products after the first two rows. The old writer selected the first two candidates before deterministic load ranking existed. The failed follow-up actually found TSS 5 kW and drafted the useful 5/6 kW comparison, but durable evidence IDs were missing and fail-closed review withheld it. That evidence-provenance failure was corrected in the earlier durable-memory work.

The retrospective also exposed a narrower remaining counterexample in the current code. Default load order ran before structured filtering, while the filter partitioned confirmed products before products with an unknown catalog attribute. In a preliminary selection, complete 6/12 kW cards could therefore precede closer 4/5 kW cards solely because the latter had an unconfirmed phase field. The final root/critic position applies the validated load order once more after the evidence partition only for `preliminary_fit`. Proven conflicts are already removed, explicit buyer ranking and remote-start preferences keep control, and `final_fit` retains confirmed-first ordering. The production-shaped regression returns 4, 5 and 6 kW and prevents 12 kW from displacing them.

PR #35's first GitHub run `34993996621` also exposed one stale test constant after the catalog timeout changed from 10 to 15 seconds. The executor behaved correctly; the test advanced the clock by only 10.001 seconds and entered primary expansion. The final test advances by the actual scheduled timeout plus one millisecond and again proves one repository call, `status=timeout`, `attempts:1` and one saved artifact.

The root challenged the critic's first conclusion that current production ordering was already sufficient. The critic accepted the confirmed/unconfirmed partition counterexample, independently passed 83/83 tests plus TypeScript and diff checks, and returned PASS with no further correction.
