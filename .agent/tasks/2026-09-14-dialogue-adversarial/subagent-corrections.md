...
## Production #2193 — comparison reference and web-target order dispute

The production draft selected DAEWOO 3.4 kW under a proven 3 kW running-only floor. The reviewer rejected the phrase that it was the nearest result by claiming ZONGSHEN 4.0 kW was closer to the 2.9 kW running load. Root challenged that verdict: `|3.4-2.9|=0.5`, while `|4.0-2.9|=1.1`. The critic agreed the reviewer was objectively wrong and that the search phrase `4–6 kW` could not become a hidden comparison reference.

Both sides agreed that code owns the numerical order after semantic policy is structured. The LLM still owns intent and explicit preferences; deterministic code filters proven conflicts, preserves evidence tiers and publishes the exact calculation used. The accepted default is known nominal power ascending above `requiredNominalKw` or the validated running-only floor, with unknown nominal values last. Explicit price/nominal objectives and remote-start preference keep their own order. The writer may state absolute nominal values and provided deltas for `returned_eligible_candidates`, but cannot turn that limited scope into a global catalogue claim.

The critic blocked the first diff because sorting the already partitioned result could move a lower-power unconfirmed candidate ahead of a fully confirmed candidate. Root adopted the critic's ordering: load sort first, then structured filtering, which preserves confirmed-before-unconfirmed and ascending power within each tier.

The critic then blocked a second integration gap. Automatic web targets were still derived from `[...productsById.values()]`; an old `priorProducts=[Energo8, DAEWOO3.4]` order survives later `Map.set` calls. Root replaced that source with the latest successful matching catalog tool result, deduplicated and intersected with the current scoped products, with map order only as a fallback. An executor regression proves the actual research call receives DAEWOO 3.4, ZONGSHEN 4, ENERGO 6.6 and FUBAG 7 even when prior products start with ENERGO 8.

Root verification after the correction: 226/226 focused tests, TypeScript, build, no-new-regex and diff checks PASS. The parallel full suite passed 1,518 tests but hit 15 five-second/resource timeouts and one worker-start timeout while build ran concurrently; the exact affected seven files passed 118/118 with one worker and a 30-second test timeout. No production code was changed for those contention-only failures.
